...
class State:
    ...
    def heuristic(self):
        return max(goal_distance[y][x] for y, x in self.positions) * (1 + epsilon) 
...
def solve():
    ...
    starting_state = State("", starting_positions, 0) 
    pq = [starting_state]
    heapq.heapify(pq)
...
def run_experiments():
    ...
    for eps in epsilon_values:
        ...
        path_length = len(solution) if solution else -1  

        results.append((eps, execution_time, path_length))

    print("\nWyniki eksperymentu:")
    print("| Epsilon | Czas wykonania (s) | Długość ścieżki |")
    print("|---------|-------------------|----------------|")
    for eps, exec_time, path_len in results:
        print(f"| {eps:<7} | {exec_time:<19.5f} | {path_len:<16} |")
