...
(define (it [code : Code] [acc : (Listof Exp)]) : Exp
  (if (empty? code)
      (first acc)
      (type-case Instr (first code)
        [(pushI n)
         (it (rest code) (append (list (numE n)) acc))]
        [(opI op)
         (it (rest code) (append (list (opE op (second acc) (first acc))) (rest (rest acc))))])))
