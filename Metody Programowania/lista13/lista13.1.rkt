    ...
    [(unboxE x)
     (let ([l (lookup-env env x)])
       ...
       (type-case Value v0
         [(refV r)
          (v*s (unbox v0) sto)]
         [else (error 'eval "try to not unbox refV")])))]
    [(set-boxE x e)
         (with [(v0 sto0) (eval e env sto)]
       (let ([x0 (lookup-env env x)])
         (type-case Value x0
           [(refV r)
            (update-sto sto0 (unboxE r) e)]
           [else
            (update-sto sto0 x0 e)])))]))
