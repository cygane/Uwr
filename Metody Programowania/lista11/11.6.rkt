...
;zmiana samego prasera
(define (parse [s : S-Exp]) : Exp
  (cond
    [(s-exp-match? `NUMBER s)
     (numE (s-exp->number s))]
    [(s-exp-match? `{lambda {SYMBOL ...} ANY} s)
     (parse-lam (s-exp->list (second (s-exp->list s)))
    ...
    [(s-exp-match? `{let SYMBOL ANY ANY} s)
     (letE (s-exp->symbol (second (s-exp->list s)))
           ...
           (parse (fourth (s-exp->list s))))]
    [(s-exp-match? `{ANY ...} s)
     (parse-app (parse (first (s-exp->list s)))
           (rest (s-exp->list s)))]
    [else (error 'parse "invalid input")]))

(define (parse-app [e1 : Exp] [e2 : (Listof S-Exp)]) : Exp
  (type-case (Listof S-Exp) e2
    [empty
     e1]
    [(cons x xs)
      (appE (parse-app e1 xs)
           (parse x))]))

(define (parse-lam [ag : (Listof S-Exp)] [e : Exp]) : Exp
  (type-case (Listof S-Exp) ag
    [empty
     e]
    [(cons x xs)
     (parse-lam (rest ag) (lamE (s-exp->symbol (first ag)) e))]))
...
(module+ test
  ...
  (test (parse `((lambda (x y) (+ x y)) 1 2))
        (appE (appE (lamE 'y (lamE 'x (opE (add) (varE 'x) (varE 'y)))) (numE 2)) (numE 1))))
