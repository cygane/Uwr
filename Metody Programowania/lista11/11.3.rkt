...
(define-type Exp
  ...
  (varE [x : Symbol])
  (letE [bi : (Listof (Symbol * Exp))] [e2 : Exp])
  (letE* [ag : (Listof (Symbol * Exp))] [e : Exp]))
    ...
    [(cons s ss)
     (if (s-exp-match? `{SYMBOL ANY} s)
         (cons (pair (s-exp->symbol (first (s-exp->list s)))
                     (parse (second (s-exp->list s))))
               (parse-let ss))
         (error 'parse-let "invalid input: let"))]))
...
(module+ test
  ...
  (test (parse `{if {= 0 1} {* 3 4} 8})
        (ifE (opE (eql) (numE 0) (numE 1))
             (opE (mul) (numE 3) (numE 4))
             (numE 8)))
  (test (parse `{let {(x 1) (y 5)} {+ x y} })
        (letE (list (values 'x (numE 1)) (values 'y (numE 5))) (opE (add) (varE 'x) (varE 'y))))
  (test (parse `{let* ([x 1]
       [y (+ x 2)]
       [z (* y 3)])
  (+ x y)})
        (letE*
 (list (values 'x (numE 1)) (values 'y (opE (add) (varE 'x) (numE 2))) (values 'z (opE (mul) (varE 'y) (numE 3))))
 (opE (add) (varE 'x) (varE 'y))))
  (test/exn (parse `{{+ 1 2}})
            "invalid input")
  (test/exn (parse `{+ 1})
            "invalid input")
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(varE x)
     (lookup-env x env)]
    [(letE bi e2)
     (eval e2 (update bi env))]
    [(letE* ag e)
     (type-case (Listof (Symbol * Exp)) ag
       [empty
        (eval e env)]
       [(cons x xs)
        (eval (letE* xs e)
              (extend-env env (fst x) (eval (snd x) env)))])]))

(define (update [xs : (Listof (Symbol * Exp))] [env : Env]) : Env
  (type-case (Listof (Symbol * Exp)) xs
    [empty
     env]
    [(cons c ss)
     (extend-env (update ss env)
                 (fst c)
                 (eval (snd c) env))]))
